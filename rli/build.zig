...
pub fn build(b: *Build) !void {
    ...
    const headers = headerFiles: {
        const genPath = try nativeCompSet.getHeader("librli");

        const writeHeader = b.addUpdateSourceFiles();

        const sourcePath = "include/rli.h";
...
fn fullBuild(b: *Build, nativeCompSet: *Compilation, dependencies: anytype, vis: SourceTree.EntryVis, optimize: std.builtin.OptimizeMode, stripDebugInfo: ?bool, t: std.Target.Query) !*FullBuild {
    ...
    const libRibbon = try comp.getLibrary("librli");
    ...
    runLibJoiner.step.dependOn(&b.addInstallArtifact(libRibbon, .{
        .dest_sub_path = b.fmt("{s}rli{s}", .{relTarget.result.libPrefix(), relTarget.result.staticLibSuffix()}),
        .dest_dir = .{
            .override = .{
                .custom = relPath,
            },
    ...
    runLibJoiner.addArgs(&[_][]const u8{ relPath, "rli", libGc.name });

    const installInclude = b.addInstallFile(
        try nativeCompSet.getHeader("librli"),
        b.fmt("{s}/include/rli.h", .{relPath}),
    );
    ...
    const bin = try comp.getBinary("rli");
